...
void dibujarTableroGrafico(SDL_Renderer* renderer) {
    ...
    // MOSTRAR EL TIEMPO DE LA PARTIDA EN FORMATO hh:mm:ss
    int totalSegundos = (int)tiempoTranscurrido;

    int horas   = totalSegundos / 3600;            // 3600 = 1 hora
    int minutos = (totalSegundos % 3600) / 60;     // minutos restantes
    int segundos= totalSegundos % 60;              // segundos restantes

    char textoTiempo[32];
    sprintf(textoTiempo, "%02d:%02d:%02d", horas, minutos, segundos);
...
void manejarClick(int x, int y) {
    ...
    if (x >= MARGIN_X && x < MARGIN_X + BOARD_SIZE * CELL_SIZE &&
        ...
        if (!esCeldaFija(fila, col)) {
            ...
            if (numeroSeleccionado > 0) {
                printf("Colocando nÃºmero %d en celda (%d, %d)\n", numeroSeleccionado, fila, col);
...
// AÑADIR ESTO: Si hay una celda seleccionada, borrar su contenido
        if (celdaSeleccionadaFila != -1 && celdaSeleccionadaColumna != -1) {
...
void dibujarPantallaVictoria(SDL_Renderer* renderer){
		...
		SDL_Color amarillo ={255,204,0,255};
		dibujarTextoCentrado(renderer,0,160,WINDOW_WIDTH,80,"VICTORIA!!",amarillo);
		//texto para las felicitaciones
		SDL_Color blanco = {255,255,255,255};
		dibujarTextoCentrado(renderer,0,250,WINDOW_WIDTH,40,"FELICITACIONES,COMPLETASTE EL SUDOKU!",blanco);
		//precione una tecla para salir
		dibujarTextoCentrado(renderer,0,350,WINDOW_WIDTH,30,"toca una tecla para finalizar",blanco);
	}
